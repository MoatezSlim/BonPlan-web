...
.filter_select:focus + .select-icon {
  color: #ffc700;
}
    ...
    <body>
        <nav id="navbar" class="navbar">
            ...
            <div class="nav_buttons">
                ...
                <button class="chat-btn" onclick="window.location.href = '/chat';"><i class="fas fa-comment"></i></button>
            </div>
        </nav>
                ...
                <div class="filter_section">
                            ...
                            <div class="filter_label_con" style="gap: 2.2vh">
                                ...
                                <select
                                    class="filter_select"
                                    id="category"
                                    name="category" onchange="handleFilterChange(this)"
                                    required>
                            ...
                            <div class="checkbox_filter" style="gap: 1.8vh">
                                ...
                                <input type="checkbox" id="offersOnly" name="offers_only"/>
                            </div>

                            <div class="checkbox_filter" style="gap: 2.5vh">
                                <label class="filter_label" for="Offers">Open Only:</label>
                                <input type="checkbox" id="openOnly" name="open_only" />
                            </div>
                            ...
                            <div class="filter-button-group">
                                <button id="filter_reset" class="filter_reset hidden" onclick="resetFilters()">Reset</button>
                                <button class="filter_submit" onclick="filterBonPlans()">Filter</button>
                            </div>
                        </fieldset>
                    </div>
                    ...
                    <fieldset class="fav_set">
                        ...
                        <div class="favorite-bonplans" id="favorite-bonplans"></div>
                    </fieldset>
                ...
                <fieldset class="map_set">
                    ...
                    <div id="canvas-container">
                        <div class="search_map">
                            <input class="search_map_field" id="filter-bonplan-map" type="search" />
                            <button class="search_button" onclick="filtrerBonPlansMap()">
                                <i class="fa-solid fa-magnifying-glass"></i>
                            </button>
                          
                          
                    @if(auth()->check() && auth()->user()->isSuperAdmin() || auth()->user()->hasRole('partenaire'))
                ...
                <form
                    ...
                    <div class="input-container">
                        ...
                        <input type="tel" id="tel_bp" name="tel_bp" required />
                                ...
                                <select class="filter_select" id="location" name="location" required>
                                    <option value="" disabled selected>Select a city</option>
                                    <option value="ariana">Ariana</option>
                                    <option value="beja">Béja</option>
                                    <option value="ben-arous">Ben Arous</option>
...
window.addEventListener("scroll", function() {
  ...
  console.log("Selected value: " + selectedValue);
    ...
    //Obtenir l'ID du User à partir du URL
    ...
    function openModal() {
        ...
        <script type="module">
            ...
            let circleCount = 0;
            let bonPlans = await getBonPlans();
            ...
            function createCircle(bonPlan) {
                ...
                // redirect to bonplan menu view when you click on a circle
                circle.addEventListener("click", function () {
                    
                    window.location.href = `/bon-plans/${bonPlan.id}/menus/user/${userId}`;
                    console.log(bonPlan.id);
                });
            }

            function renderCircles(bps) {
                clearAllCircles();
                for (const bonPlan of bps) {
            ...
            // render bonplans on initial load
            renderCircles(bonPlans);
            ...
            window.filterBonPlans = async function() {
                const category = document.getElementById("category").value;
                const city = document.getElementById("city").value;
                const offersOnly = Boolean(document.getElementById("offersOnly").checked);
                const openOnly = Boolean(document.getElementById("openOnly").checked);
                ...
                if (offersOnly) {
                    queryString.append("offers_only", offersOnly);
                }

                if (openOnly) {
                    queryString.append("open_only", openOnly);
                }

                if (rating) {
                    queryString.append("min_rating", rating);
                }

                // request API
                const response = await fetch('/api/bon-plans/filter?' + queryString, {
                    method: "GET",
                    headers: {
                ...
                if (!response.ok) {
                    throw new Error('Failed to filter BonPlans');
                }

                // re-render with new results
                bonPlans = await response.json();
                renderCircles(bonPlans);

                // update filtered state if values aren't equel to their default
                if (category.length || city.length || offersOnly || openOnly || rating != 0)
                    updateFilteredState(true);
            }
            ...
            window.resetFilters = async function() {
                ...
                document.getElementById("offersOnly").checked = false;
                document.getElementById("openOnly").checked = false;
                document.getElementById("rating").value = 0;

                // request API
                bonPlans = await getBonPlans();

                // re-render with new results
                renderCircles(bonPlans);

                updateFilteredState(false);
            }

            function updateFilteredState(state) {
                isFiltered = state;

                const resetFilterButton = document.getElementById("filter_reset");
            ...
            /* filtrage map */
            window.filtrerBonPlansMap = function() {
                const nom_bp = document.getElementById("filter-bonplan-map").value;
                let filteredBonPlans = nom_bp.length 
                    ? bonPlans.filter((bonPlan) => bonPlan.nom_bp.toLowerCase().includes(nom_bp.toLowerCase()))
                    : bonPlans;

                renderCircles(filteredBonPlans);
            }
            ...
            async function getFavoriteBonPlans() {
                try {
                    const response = await fetch(`/api/bon-plans/favorites/${getUserIdFromUrl()}`);
                    if (!response.ok) {
                        throw new Error('Failed to fetch favorite BonPlans');
                    }
                    const favoriteBonPlanIds = await response.json();
                    const favoriteBonPlans = bonPlans.filter((bonPlan) => favoriteBonPlanIds.includes(bonPlan.id));
                    return favoriteBonPlans;
                } catch (error) {
                    console.error('Error fetching favorite BonPlans:', error.message);
                }
            }

            const favoriteBonPlans = await getFavoriteBonPlans();
            
            const favoriteBonPlansContainer = document.getElementById("favorite-bonplans");

            // add circles

            function createFavoriteCircle(bonPlan) {
                const category = processProperties(bonPlan.categorie_bp);

                const circle = document.createElement("div");
                circle.classList.add("circle-favorite");

                circle.style.backgroundColor = category.color;
                circle.title = bonPlan.nom_bp;
                circle.innerHTML = `<i class="icon fas ${category.icon}"></i>`;
                circle.setAttribute("id", bonPlan.id);
                favoriteBonPlansContainer.appendChild(circle);
            ...
            for (const bonPlan of favoriteBonPlans) {
                createFavoriteCircle(bonPlan);
            }
        </script>
    </body>
</html>